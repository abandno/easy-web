package com.nisus.baotool.util.reflect;

import com.google.common.base.CaseFormat;
import com.nisus.baotool.core.enums.CaseTransform;
 ...
 * 字段名工具
 * <p>
 * 1. 根据 getter lambda 获取字段名.
 * 2. 解析 getter|setter 对应属性名.
 *
 * @author L&J
 * @date 2021/10/2 4:12 下午
 */
public class FieldNameUtil {
     ...
     * @return
     */
    public static <T> String get(SFunction<T, ?> accessor) {
        String implMethodName = LambdaUtil.getSerializedLambda(accessor).getImplMethodName();
        return fromAccessor(implMethodName);
    }

    /**
     * {@code get(accessor, toFormat, CaseFormat.LOWER_CAMEL)} 快捷方式
     *
     * @param accessor
     * @param toFormat
     * @param <T>
     ...
     * @param accessor  getter
     * @param <T>       bean 类型
     * @param srcFormat 原 format
     * @param toFormat  转为 format
     * @return String
     */
     ...
     *
     * @param accessor getter|setter 方法名
     * @return
     */
        ...
        */
        if (accessor.startsWith(PREFIX_IS)) {
            ...
            if (!accessor.startsWith(PREFIX_GET) && !accessor.startsWith(PREFIX_SET)) {
                throw new BRuntimeException("Error parsing property methodName '" + accessor + "'.  Didn't start with 'is', 'get' or 'set'.");
            }
