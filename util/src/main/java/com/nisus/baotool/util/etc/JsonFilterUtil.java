...
public class JsonFilterUtil {
    private static final Logger log = LoggerFactory.getLogger(JsonFilterUtil.class);
    ...
    public static JsonValueFilter valueFilter(FilterConfig config) {
    ...
    public static class JsonValueFilter implements ValueFilter {
        public static final JsonValueFilter DEFAULT = new JsonValueFilter(FilterConfig.DEFAULT);

        @Setter
        private FilterConfig config;

        public JsonValueFilter(FilterConfig config) {
        ...
        public Object process(Object o, String name, Object value) {
            ...
            try {
                ...
                log.error("截取超长字段异常 name: " + name, e);
            }
        ...
        private String truncateString(String name, String value) {
            ...
            if (value.startsWith(BASE64_IMG_PREFIX)) {
                log.info("截取base64字段: {}, type: {}, length: {}", name, type,
                        value.length());
            ...
            // json 串
                ...
                if (value.length() > config.jsonStringMaxLen) {
                    log.info("截取超长JSON字符串字段: {}, type: {}, length: {}", name, type,
                            value.length());
            ...
            // 普通字符串
                ...
                log.info("截取超长字段: {}, type: {}, length: {}", name, type,
                        value.length());
        ...
        private Object truncateJsonArray(String name, Object value) {
            if (value instanceof Collection) {
                ...
                if (i > config.arrayMaxSize) {
                    log.info("截取超长 Collection: {}, type: {}, length: {}", name, "Collection",
                            value1.size());
                }
                ...
                if (len > config.arrayMaxSize) {
                    log.info("截取超长 Array: {}, type: {}, length: {}", name, "Collection",
                            len);
                }
        ...
        private static boolean likeJson(@NotNull String src) {
            boolean maybe = (src.startsWith(JSON_OBJ_START_KEY) && src.endsWith(JSON_OBJ_END_KEY))
                    || (src.startsWith(JSON_ARR_START_KEY) && src.endsWith(JSON_ARR_END_KEY));

            if (maybe) {
                return JSONValidator.from(src).validate();
            }
    ...
    public static String toJSONString(Object obj, FilterConfig config, SerializerFeature... features) {
