...
public class EmptyUtil {
    ...
    public interface EmptyMode {
        ...
        /**
         * 目标值空时, 返回指定的 defaultValue
         * @param subject      目标值
         * @param defaultValue 空时的降级默认值
         * @param otherEmptyCases   其他自定义空case
         * @param <T>          目标值类型
         * @return 是否空
         */
        default <T> T defaultIfEmpty(final T subject, final T defaultValue, Object... otherEmptyCases) {
            return this.isEmpty(subject, otherEmptyCases) ? defaultValue : subject;
        }
    }
    ...
    private static class ExtendEmptyMode implements EmptyMode {
        ...
        public boolean isEmpty(Object subject, Object... otherEmptyCases) {
            ...
            if (flg) {
                // 已经判断是 空, 下面不用在判断了, 用户指定的优先级更高
                return flg;
            }

            if (subject instanceof CharSequence) {
                // 'NULL' 算空吗?
                flg = StrUtil.isBlank((CharSequence) subject) || "null".equals(subject);
