...
import com.nisus.baotool.core.ext.lang.AbstractFlag;
 ...
 * 注: 多 Numbor 运算时, 始终<em>以调用方规则为准</em>. 链式调用时,
 * 第一个 Numbor rule 就是所有 Numbor 统一使用的规则.
 * 所以, 建议使用一致的规则, 否则可能结果非预预期.
...
public class Numbor /*extends Number*/ implements Comparable<Numbor>, Serializable {
     ...
     * 默认
     * <pre>
     * - 不忽略特殊值: null, 无穷大, 无穷小, 非数字 参与运算.
     * - 特殊值不转换: null 还是 null; 无穷值还是无穷值; 非数字还是非数字.
     * </pre>
     */
     ...
     * <pre>
     * - 不忽略特殊值: null, 无穷大, 无穷小, 非数字 参与运算.
     * - 特殊值转换: null 当作 0; 无穷值 当作 0; 非数字 当作 0.
     * </pre>
     */
    ...
    private Numbor tryCalculate(Numbor other, BiFunction<Number, Number, Number> calculator, BiFunction<Double, Double, Number> onException) {
        // // other 对象 null, 忽略 --> null 也由 null 相关规则控制
        // if (other == null) {
        //     return this;
        // }
        other = wrap(other);

        // 异常值忽略处理, 如果设置了对应策略, 且刚好有对应异常值, 则忽略, 不运算
        ...
        if (this.isEmpty() || other.isEmpty()) { // here, 不忽略null, 但遇到null, 返回 EMPTY
            // return EMPTY;
            return isEmpty() ? this : other; // ! 因为这里要返回 '空' 语义的, 所以谁 空, 应该返回谁 !
        }
    ...
    /**
     * 如果 Rule 不一致, 则统一应用 this.rule + 原始值 构建新实例
     */
    private Numbor wrap(Numbor other) {
        // return new Numbor(origin != null ? origin.orElse(null) : null, rule);
        if (other == null) {
            return new Numbor(null, rule);
        }
        ...
        // 取原始 value, 绕过 rule
        return new Numbor(other.value, rule);
    }
